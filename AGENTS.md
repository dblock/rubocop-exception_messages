...
* README examples use the `raise Class, "message"` form for brevity; don't repeat the `raise Class.new("message")` form in every example. Both forms are recognized by every cop, so document this once rather than in each cop's section. Show both forms only where the choice between them is what the cop itself checks (e.g. `ExceptionMessages/RequireMessage`).
